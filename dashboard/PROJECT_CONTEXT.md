...
- 2026-04-27: Foi iniciada a funcionalidade de `Ciclos de cultivo` com um MVP orientado a simplicidade e utilidade. O sistema ganhou o modelo `CultivationCycle`, migração Alembic própria, serviço `apps/services/cycle_service.py`, rotas para iniciar/finalizar ciclos em `apps/home/routes.py`, a nova página `templates/home/cycles.html` e um item `Ciclos` na sidebar. Nesta primeira fase, o ciclo é tratado como uma janela de tempo sobre as leituras já existentes: o dashboard calcula quantidade de leituras, sensores com dados, duração do ciclo e resumo por sensor (mínimo, média, máximo e último valor) sem precisar reestruturar a tabela `sensor_readings`. A migração local foi aplicada com sucesso via `.venv/bin/python -m flask db upgrade` e a rota `/cycles.html` foi validada com `test_client` retornando `200`.
- 2026-04-27: A página `templates/home/cycles.html` teve o tema alinhado ao restante do dashboard. A interface passou a adotar a mesma linguagem visual consolidada em `Sistema` e `Análise de biomassa`, com hierarquia de kicker/título mais próxima das outras telas, cards com `section-shell` interno, melhor ritmo entre blocos e contraste consistente nos temas escuro e claro. A mudança foi intencionalmente visual, sem alterar o fluxo funcional do MVP de ciclos.
- 2026-04-27: O botão de ação secundária da página `Ciclos` foi corrigido para a paleta do produto. O estilo padrão `btn-outline-primary` estava puxando o azul herdado do tema base, então `templates/home/cycles.html` recebeu a variante local `cycle-outline-btn`, alinhada ao verde/ciano já usado no restante do dashboard.
- 2026-04-27: O ajuste inicial da cor do botão secundário de `Ciclos` não foi suficiente porque o tema base ainda estava impondo a paleta rosa em algumas regras de botão. A correção foi reforçada em `templates/home/cycles.html` com seletores mais específicos (`.btn.cycle-outline-btn`) e `!important` apenas nessa ação local, garantindo a paleta verde/ciano tanto no tema escuro quanto no claro.
- 2026-04-27: O botão principal `Iniciar ciclo` da página `Ciclos` foi alinhado à cor primária verde do produto. Antes ele ainda usava `btn-primary` do tema base, puxando a paleta rosa do Black Dashboard. `templates/home/cycles.html` recebeu a variante local `cycle-primary-btn`, com override explícito para os temas escuro e claro.
- 2026-04-27: A tipografia da página `Ciclos` foi refinada com foco em legibilidade. `templates/home/cycles.html` recebeu melhor contraste em labels e textos auxiliares, títulos de card ligeiramente mais firmes, campos de formulário com leitura mais clara, valores de meta/estatística com linha e peso mais equilibrados e tabelas com hierarquia tipográfica mais nítida. O objetivo foi aumentar a visibilidade de áreas como `Nome do ciclo` e `Observações` sem descaracterizar a linguagem visual já consolidada no dashboard.
- 2026-04-27: A segunda iteração funcional da página `Ciclos` foi concluída. `apps/services/cycle_service.py` passou a calcular amplitude por sensor, classificar estabilidade (`Estável`, `Moderado`, `Oscilante`) e gerar um pequeno bloco de insights do ciclo (`sensores estáveis`, `sensores oscilantes` e `sensor com maior amplitude`). Em `apps/home/routes.py`, esses insights passaram a ser serializados para a UI. Já `templates/home/cycles.html` foi reorganizada para dar mais protagonismo ao ciclo ativo, enriquecer o histórico com data inicial e leituras, e tornar o resumo menos cru com amplitude e estabilidade por sensor. A rota `/cycles.html` seguiu validada com `test_client`, retornando `200`.
- 2026-04-28: Foi corrigido um `IntegrityError` ao iniciar ciclos de cultivo. A falha era `NOT NULL constraint failed: cultivation_cycles.created_at`, reproduzida ao testar `CycleService.start_cycle(...)` via `.venv/bin/python`. A causa estava na combinação entre o schema SQLite já migrado e o modelo `CultivationCycle`, que confiava apenas em `server_default` para `created_at` e `updated_at`. A correção foi aplicada em `apps/authentication/models.py`, adicionando `default=lambda: datetime.now(timezone.utc)` para `started_at`, `created_at` e `updated_at`, além de `onupdate` explícito em `updated_at`. A migração `a31f62b84e11_add_cultivation_cycles.py` também foi endurecida para novos bancos com `server_default=sa.text('(CURRENT_TIMESTAMP)')` nesses campos. Após a correção, a criação e remoção de um ciclo de teste voltou a funcionar normalmente.
- 2026-04-28: A página `Ciclos` passou por uma revisão de UX motivada por três problemas observados em uso real: sensação de histórico “bugado” por conta de rolagem, redundância entre `Ciclo atual` e `Resumo do ciclo`, e ausência de clareza quando um ciclo é criado sem leituras no intervalo. Em `templates/home/cycles.html`, o histórico em tabela foi substituído por uma lista de cards compactos, eliminando a rolagem lateral/vertical do bloco e deixando as ações mais diretas. O card `Ciclo atual` foi reduzido ao papel operacional (estado, métricas rápidas, observações e ações), enquanto o `Resumo do ciclo` passou a concentrar a leitura analítica. A antiga tabela de sensores foi trocada por cards por sensor, com nome explícito, leituras, mínimo, média, máximo, amplitude, última leitura e estabilidade, evitando overflow horizontal. Também foi adicionada a ação de exclusão de ciclo via `apps/services/cycle_service.py` e `apps/home/routes.py`. Na investigação do caso reportado, foi confirmado via consulta local que o único ciclo de teste existente (`Ciclo-teste-abril`) tinha `reading_count = 0`, então a ausência de sensores e métricas no resumo era coerente com os dados; por isso a tela agora explica explicitamente esse estado vazio ao usuário. A rota `/cycles.html` foi validada novamente com `test_client`, retornando `200`.
- 2026-04-28: O histórico da página `Ciclos` foi enxugado após revisão de UX. Os metadados secundários sob o título do ciclo foram reduzidos para apenas data e duração, diminuindo a altura de cada card e evitando que o bloco chamasse atenção demais. Em `templates/home/cycles.html`, os controles `Ver resumo` e `Excluir` também foram harmonizados: passaram a compartilhar altura percebida, alinhamento vertical e espaçamento mais consistente, deixando o conjunto mais limpo e equilibrado visualmente.
- 2026-04-28: O módulo `Ciclos` ganhou uma segunda visão para escalar o histórico sem sobrecarregar a tela principal. `apps/services/cycle_service.py` passou a expor `list_cycles_paginated(page, per_page)`, `apps/home/routes.py` ganhou `build_cycles_history_context(request)` e `route_template()` passou a montar contexto também para `cycles_history.html`. Em `templates/home/cycles.html`, o bloco `Histórico recente` ganhou um link `Ver todos`, mantendo o card enxuto e focado só nos ciclos recentes. A nova página `templates/home/cycles_history.html` foi criada com paginação básica, resumo total de ciclos, ações de `Ver resumo` e `Excluir`, além de navegação entre páginas. O fluxo de exclusão também foi refinado para respeitar a origem da ação: quando um ciclo é apagado a partir do histórico completo, a interface permanece nessa própria visão paginada. Após um ajuste de contexto para evitar conflito do Jinja com a chave `items`, as rotas `/cycles.html` e `/cycles_history.html` foram validadas com `test_client`, ambas retornando `200`.
- 2026-04-28: A página `Todos os ciclos` foi refinada com filtros e alinhamento visual melhor no histórico completo. Em `templates/home/cycles_history.html`, o chip de status (`Em andamento` / `Finalizado`) passou a se alinhar corretamente à linha de metadados do card, evitando a sensação de elemento “caído” na interface. A mesma tela também ganhou filtros simples e úteis: `status`, `início a partir de` e `início até`. No backend, `apps/services/cycle_service.py` passou a suportar paginação filtrada por status e intervalo de `started_at`, enquanto `apps/home/routes.py` passou a normalizar esses parâmetros e preservá-los entre paginação e exclusão de ciclos. A renderização da página foi validada com `test_client` nas combinações sem filtro, com filtro por status e com filtro por intervalo de datas, todas retornando `200`.
- 2026-04-28: As ações destrutivas do módulo `Ciclos` passaram a pedir confirmação antes do envio. Em `templates/home/cycles.html` e `templates/home/cycles_history.html`, os formulários de `Finalizar ciclo` e `Excluir` agora usam confirmação nativa via `onsubmit` + `confirm(...)`, reduzindo risco de misclick sem introduzir modal customizado ou complexidade extra no protótipo.
- 2026-04-28: A confirmação nativa do navegador para `Finalizar` e `Excluir` em `Ciclos` foi substituída por modais próprios e centralizados em `templates/home/cycles.html` e `templates/home/cycles_history.html`. A motivação foi melhorar o enquadramento visual e deixar a confirmação mais coerente com a interface do dashboard, já que `confirm()` não permite controle de posicionamento ou aparência. Os formulários passaram a usar a classe `cycle-confirm-form` com `data-confirm-title` e `data-confirm-message`, enquanto cada página ganhou seu próprio backdrop/modal e um script leve para interceptar o submit, abrir a confirmação centralizada e só então reenviar a ação. As duas páginas foram renderizadas com `test_client`, ambas retornando `200` e contendo o novo markup de modal.
- 2026-04-28: O contraste dos feedbacks de sucesso no módulo `Ciclos` foi reforçado. Em `templates/home/cycles.html` e `templates/home/cycles_history.html`, as caixas `.feedback-box.is-success` passaram a usar texto mais claro no tema escuro e verde mais escuro/legível no tema claro, para mensagens como `Ciclo excluído com sucesso.` não parecerem apagadas.
- 2026-04-28: A página `Análise de biomassa` foi alinhada estruturalmente com `Sistema` e `Ciclos`. Em `templates/home/img_rec.html`, o card mais externo do topo (`hero-card workspace-card`) foi removido e a introdução passou a viver diretamente no background da página, via `page-intro`, reduzindo poluição visual e deixando a hierarquia mais próxima das outras telas do dashboard. O shell de trabalho permaneceu como card interno, preservando o foco no upload/preview/resultado. A lógica de scroll e feedback também foi atualizada para usar o novo container `img-rec-intro` em vez do antigo `hero-card`. A rota `/img_rec.html` foi validada com `test_client`, retornando `200`.

- 2026-04-28: Simplifiquei a hierarquia visual da página `img_rec` removendo a casca redundante dos blocos de upload e resultado. A introdução ficou solta no background da página, enquanto `workspace-shell` e `result-shell` passaram a ser os únicos contêineres visuais principais, evitando o efeito de card duplo e reduzindo o desalinhamento do resultado. Validei `/img_rec.html` com `test_client` retornando `200`.
- 2026-04-28: Refinei novamente a página `img_rec` para reduzir ruído visual e melhorar o fluxo de análise. `workspace-shell` e `result-shell` ganharam mais presença visual, o bloco de resultado passou a abrir com maior respiro em relação ao upload, e o botão `Analisar biomassa` agora some após uma análise bem-sucedida, reaparecendo apenas quando o resultado é invalidado por nova imagem ou mudança de ROI. Validei `/img_rec.html` com `test_client` retornando `200`.
- 2026-04-28: Corrigi a exibição dos horários de ciclos para o fuso de Manaus (America/Manaus). Os ciclos continuam sendo salvos em UTC no backend, mas agora `cycles.html` e `cycles_history.html` recebem horários já convertidos na serialização da rota. Também ajustei os filtros por data do histórico para interpretar o intervalo como dia local de Manaus antes de converter para UTC. Validei `/cycles.html` e `/cycles_history.html?status=finished&date_from=2026-04-28&date_to=2026-04-28` com `test_client`, ambos retornando `200`.
